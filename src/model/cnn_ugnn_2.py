...
class CnnGegn(BaseGegn):
    def __init__(self,
        ...
        print("This is cnn_2.")
        self.one_hot_hour = config.one_hot_hour
        ...
        self.o_out_fcnn_layer = self._make_layer(LinearBlock,
                                                 ...
                                                 config.o_grid_fcnn_layer_channels[-1] + config.o_grid_extra_fcnn_layer_channels[-1],
                                                 False)

        self.d_out_fcnn_layer = self._make_layer(LinearBlock,
                                                 config.d_grid_out_fcnn_layer_channels,
                                                 config.d_grid_fcnn_layer_channels[-1] + config.d_grid_extra_fcnn_layer_channels[-1],
                                                 False)

        # Dummy hour layer
        if config.one_hot_hour:
            self.hour_dim = config.hour_dim
            
            self.one_hot_hour_layer = self._make_layer(LinearBlock,
                                                config.hour_channels,
                                                config.hour_dim,
                                                False)
            self.others_layer = self._make_layer(LinearBlock,
                                                config.others_channels,
                                                config.others_dim,
                                                False)
    ...
    def forward(self, o_grid_basic_x, d_grid_basic_x, o_grid_extra_x, d_grid_extra_x, edge_x, external_x):
        ...
        if self.one_hot_hour:
            hour_out = self.one_hot_hour_layer(edge_x[:, :self.hour_dim])
            others_out = self.others_layer(edge_x[:, self.hour_dim:])
            edge_out = self.edge_layer(torch.cat([hour_out, others_out], 1))
        else:
