...
class GridPreprocessor(object):
    ...
    def __call__(self):
        ...
        normalized_all_grid_df = util.enlarge_grid(raw_all_grid_df[['id']], normalized_all_grid_df, self.grid_radius)
        
        # Here only model with name cnn will be reshaped as square.
        reshaped_normalized_all_grid_dict = util.grid_reshape(normalized_all_grid_df, self.model_name, grid_dim,
                                                            self.grid_num, self.grid_radius)
