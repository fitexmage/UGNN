...
from src.config.ugnn_gat_config import SAGEGegnConfig
from src.model.ugnn_gat import SAGEGegn
from src.utils.dataset import SAGEDataset
import util
...
def train(p_data, model, edge_preprocessor, **kwargs):
    def save_model(path):
    ...
    def eval(best_loss):
        ...
        valid_dataset = SAGEDataset(kwargs['o_idx'], kwargs['d_idx'], kwargs['id_dict'], p_data["valid_edge"],config.one_hot_hour, kwargs['edge_pair'])
        ...
        for i, valid_data in enumerate(valid_loader):
            ...
            y_pred = model(*valid_data[:-1], kwargs['x_concat'], kwargs['edge_index'], kwargs['id_dict'])
        ...
        y_pred = torch.cat(y_preds, dim=0).squeeze(-1)
    ...
    kwargs['x_concat'] = kwargs['x_concat'].to(config.device)
    kwargs['edge_index'] = kwargs['edge_index'].to(config.device)
    ...
    for epoch in range(config.epoch):
        ...
        if epoch % config.sample_gap == 0:
            ...
            train_dataset = SAGEDataset(kwargs['o_idx'], kwargs['d_idx'], kwargs['id_dict'],data[-1], config.one_hot_hour, kwargs['edge_pair'])
        ...
        for i_batch, train_data in enumerate(train_dataloader):
            ...
            y_pred = model(*train_data[:-1], kwargs['x_concat'], kwargs['edge_index'], kwargs['id_dict']).squeeze(-1)
...
def test(cities, model, note, **kwargs):
    ...
    for city in cities:
        ...
        test_dataset = SAGEDataset(kwargs['o_idx'], kwargs['d_idx'], kwargs['id_dict'], p_data['test_edge'], config.one_hot_hour, kwargs['edge_pair'])
...
def get_gat_data(p_data, model_status):
    if model_status == 'train':
        id_list = []
        for i in config.train_cities:
            id_list += p_data['train_id'][i]
            id_list += p_data['valid_id'][i]
    else:
        id_list = []
        for i in config.test_cities:
            id_list += p_data['train_id'][i]
            id_list += p_data['valid_id'][i]
    id_dict = {}
    ...
    for i in range(len(id_list)):
        for direction in directions:
            neighbor = util.add_to_id1000(int(id_list[i]), direction)
            if neighbor in id_list:
                o_ids.append(int(id_list[i]))
                d_ids.append(neighbor)
    
    
    x1 = []
    for key in id_dict.keys():
        x1.append(p_data['grid']['grid_basic_dict'][key].reshape([1,7]))
    x1 = torch.concat(x1)
    
    x2 = []
    for key in id_dict.keys():
        x2.append(p_data['grid']['grid_extra_dict'][key].reshape([1,42]))
    x2 = torch.concat(x2)
    
    o_idx = [id_dict[i] for i in o_ids]
    d_idx = [id_dict[i] for i in d_ids]
    x_concat = torch.cat((x1, x2), dim=1)
    
    # Make edge index
    edge_pair = [(o_idx[i], d_idx[i]) for i in o_idx]
    edge_index = torch.tensor([o_idx, d_idx])
    gat_data = {}
    gat_data['id_dict'] = id_dict
    gat_data['o_idx'] = o_idx
    gat_data['d_idx'] = d_idx
    gat_data['edge_index'] = edge_index
    gat_data['x_concat'] = x_concat
    gat_data['edge_pair'] = edge_pair
    return gat_data
...
def main():    
    if config.model_status == "train":
        ...
        gat_data = get_gat_data(p_data, config.model_status)
        train(p_data, model, edge_preprocessor, **gat_data)
        ...
        model, criterion, optimizer= setup_model(model)
        gat_data = get_gat_data(p_data, config.model_status)
        test(cities, model, config.note, **gat_data)
    else:
...
if __name__ == "__main__":
    ...
    config = SAGEGegnConfig()
    config.grid_radius = 1
